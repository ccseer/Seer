...
// not sure if making "static" will speed up conversion
...
bool QPsdHandler::read(QImage *image)
{
    ...
    input
        >> height;  // Supported range is 1 to 30,000. (**PSB** max of 300,000.)
    if (version == 1 && (height > 30000 || height == 0))
        return false;
    if (version == 2 && (height > 300000 || height == 0))
        return false;

    input >> width;  // Supported range is 1 to 30,000. (**PSB** max of
                     // 300,000.)
    ...
    switch (compression) {
        ...
        {
            /*  This algorithm might be inaccurate and was based ONLY
        ...
        {
            /* The RLE-compressed data is proceeded by a 2-byte(psd) or
             * 4-byte(psb)
            ...
            switch (depth) {
                case 8:
                    switch (channels) {
                        ...
                        default:  // excess channels other than Gray are
                                  // considered alphas
                            {
                                QImage result(width, height,
            ...
            switch (depth) {
                case 8:
                    switch (channels) {
                        ...
                        default:  // excess channels other than RGB are
                                  // considered alphas
                            {
                                QImage result(width, height,
                                              QImage::Format_ARGB32);
                                ...
                                for (quint32 y = 0; y < height; ++y) {
                                    ...
                                    while (p < end) {
                                        ...
                                        if (*alpha != 0) {
                                            ...
                                            *p = qRgba(rFixed, gFixed, bFixed,
                                                       a);
                                        }
                                        else {
                                            *p = qRgba(*red, *green, *blue,
                                                       *alpha);
                                        }
                                    ...
                                    while (p < end) {
                                        ...
                                        alpha16
                                            = ((*alpha8 << 8) + *(alpha8 + 1))
                                              * scale;
                                        *p = qRgba(
                                            (quint8)red16, (quint8)green16,
                                            (quint8)blue16, (quint8)alpha16);
                                        ++p;
                                        red8 += 2;
                                        green8 += 2;
                                        blue8 += 2;
            ...
            switch (depth) {
                case 8: {
                    switch (channels) {
                         ...
                         * Converting a CMYK image to Multichannel mode creates
                         * cyan, magenta, yellow, and black spot channels.
                         * Converting an RGB image to Multichannel mode creates
                         * cyan, magenta, and yellow spot channels.
                         */
                        case 3: {
                            QImage result(width, height, QImage::Format_RGB32);
                        ...
                        default:  // excess channels other than CMYK are
                                  // considered alphas
                            {
                                QImage result(width, height,
                                              QImage::Format_ARGB32);
            ...
            switch (depth) {
                case 8:
                    switch (channels) {
                        case 1: {
                            /*
                             *Duotone images: color data contains the duotone
                             *specification
                             *(the format of which is not documented). Other
                             *applications that
                             *read Photoshop files can treat a duotone image as
                             *a gray image,
                             *and just preserve the contents of the duotone
                             *information when
